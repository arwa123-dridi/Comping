...
public class StatistiquesAvisMapper {
        ...
        public static StatistiquesAvisDTO toDTO(List<Avis> avisList) {
                ...
                long nombre5Etoiles = avisList.stream().filter(a -> a.getNote() == 5).count();
                long nombre4Etoiles = avisList.stream().filter(a -> a.getNote() == 4).count();
                long nombre3Etoiles = avisList.stream().filter(a -> a.getNote() == 3).count();
                long nombre2Etoiles = avisList.stream().filter(a -> a.getNote() == 2).count();
                long nombre1Etoile = avisList.stream().filter(a -> a.getNote() == 1).count();
         ...
         */
        public static double calculerPourcentage(long nombreNote, long nombreTotal) {
                if (nombreTotal == 0) {
                        return 0.0;
                }
